...
public class Board {
	...
	/**
	 * Coloca uma peça do player na posição (row,col). Não depende das regras do
	 * jogo, apenas tem que estar dentro do tabuleiro.
	 * 
	 * @param Piece
	...
	/**
	 * Retira a peça do tabuleiro. Na prática apenas atribui à peça o jogador -1
	 * que representa a ausência de jogador
	 * 
	 * @param row
	 * @param col
	 */
	...
	/**
	 * Coloca uma peça do player na posição (row,col). Não depende das regras do
	 * jogo, apenas tem que ser uma posição livre.
	 * 
	 * @param Piece
	 ...
	 * encontrar usa o {@link #auxwinnerWhite(int row, int col, boolean[][] a)
	 ...
	 * 
	 * @param row
	 *            linha que está a tratar neste momento
	 * @param col
	 *            coluna que está a ser tratado neste momento
	 * @return se é promising ou não através de bool, true se continuar, false
	 *         se não for util continuar por este caminho
	 */
	public Boolean auxwinnerWhite(int row, int col) {
		...
		if (board[row][col].getPlayer() == PLAYER_WHITE && !visited[row][col]) {
			...
			// Verificar posiï¿½ï¿½es ortogonais
			...
			// Verificar posiï¿½ï¿½es diagonais
	 ...
	 * {@link #auxwinnerBlack(int row, int col, boolean[][] a) auxwinnerBlack}
